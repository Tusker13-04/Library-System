...
# if bookrev.csv file exists then read it else create a new file
if os.path.exists("bookrev.csv"):
    pass
else:
    ...
    df = pd.DataFrame()
    df.to_csv('bookrev.csv', index=False)
...
# with open("bookrev.csv", 'w', newline='') as f:
#    write = csv.writer(f)
...
def adding():
    # adding new record to csv VIa dataframe
    df = pd.read_csv('bookrev.csv')
    print(df)
    print("Enter the details of the book you want to add")
    no_of_records = int(input("Enter the number of books you want to add: "))
    for i in range(no_of_records):
        ...
        if book_id in df['book_id'].values:
            print("The book id already exists. Please enter a new book id")
            book_id = int(input("Enter the book id: "))
        # check if the book name already exists  case insensitive
        elif book_name.lower() in df['book_name'].str.lower().values:
            print("The book name already exists. Please enter a new book name")
            book_name = input("Enter the book name: ")
        else:
            df.loc[len(df)] = [book_id, book_name, author_name, rating, genre, review]
            print("The book details have been added successfully")
    df.to_csv('bookrev.csv', index=False)
    print(df)

    #
    #    # check if book name already exists under column book in csv file ignore case sensitivity
    #    with open("bookrev.csv", 'r') as f:
    #        reader = csv.reader(f)
    #        for row in reader:
    #
...
# reading a review
def ask():
    # reading a review from csv file
    df = pd.read_csv('bookrev.csv')
    print(df)
    book_name = input("Enter the book name: ")
    # check if the book name exists in the csv file
    if book_name.lower() in df['book_name'].str.lower().values:
        print(df[df['book_name'].str.lower() == book_name.lower()])
        # print the review
        print("The review is: ", df[df['book_name'].str.lower() == book_name.lower()]['review'].values[0])

    else:
        print("The book name does not exist")
    main()
